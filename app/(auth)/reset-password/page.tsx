  ...
  return (
    <div style={{ width: '100%', maxWidth: 420, padding: '0 24px', animation: 'fadeUp 0.5s ease' }}>
      ...
      {done ? (
        ...
        <div style={{ textAlign: 'center', padding: '24px', background: '#FEE2E2', borderRadius: 12, border: '1px solid #FECACA' }}>
          <div style={{ fontSize: 32, marginBottom: 8 }}>❌</div>
          <div style={{ fontSize: 14, color: '#DC2626' }}>{error}</div>
          <button onClick={() => router.push('/login')} style={{ marginTop: 16, fontSize: 13, color: '#2563EB', background: 'none', border: 'none', cursor: 'pointer' }}>← Back to login</button>
        </div>
      ) : !ready ? (
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: 32, boxShadow: '0 4px 24px rgba(0,0,0,0.06)', border: '1px solid #E2E8F0' }}>
          ...
          <p style={{ fontSize: 13, color: '#94A3B8', textAlign: 'center', marginTop: 16, cursor: 'pointer' }} onClick={() => router.push('/login')}>← Back to login</p>
        </div>
      )}
    </div>
  )
...
export default function ResetPasswordPage() {
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: '"SF Pro Display",-apple-system,BlinkMacSystemFont,sans-serif', background: '#FAFAF8', alignItems: 'center', justifyContent: 'center' }}>
      ...
      `}</style>
      <Suspense fallback={<div style={{color:'#94A3B8',fontSize:14}}>Loading...</div>}>
        <ResetForm />
      </Suspense>
    </div>
  )
